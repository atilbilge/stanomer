-- ============================================================================
-- STANOMER PRODUCTION DATABASE MIGRATION & SYNC SCRIPT
-- Safe & Idempotent script for Production Supabase (ustcsvvkzsmsgzbptvpm)
-- Last Updated: 2026-08-13 — Added email_unsubscribes table
-- Will NOT delete any existing data. Only adds missing columns, tables & RLS policies.
-- ============================================================================

-- 1. ENUM TYPES (Safely created if missing)
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN
        CREATE TYPE public.user_role AS ENUM ('landlord', 'tenant', 'both', 'agency');
    END IF;
    -- Add 'agency' to existing ENUM if missing (idempotent)
    IF NOT EXISTS (
...
-- 2. ENSURE ALL TABLES & COLUMNS ARE PRESENT IN PRODUCTION
...
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS active_role TEXT DEFAULT 'landlord';
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS role public.user_role DEFAULT 'landlord';
...
-- 2.7 notifications & activity_logs
...
--- 3. RLS POLICIES CLEANUP & RECREATION (with agency support)
...
-- Contracts RLS (with agency support and token lookup)
...
-- 3.3 Helper functions (SECURITY DEFINER) to break RLS circular recursion
...
CREATE POLICY "invitations_select_policy" ON public.invitations FOR SELECT TO authenticated
    USING (inviter_id = auth.uid() OR public.is_agency_of_property(property_id, auth.uid()) OR LOWER(invitee_email) = LOWER(COALESCE(auth.jwt()->>'email', '')) OR (token IS NOT NULL));
...
CREATE POLICY "invitations_insert_policy" ON public.invitations FOR INSERT TO authenticated
    WITH CHECK (inviter_id = auth.uid() OR public.is_agency_of_property(property_id, auth.uid()));

-- 3.5 rent_payments RLS
...
CREATE POLICY "rent_payments_select_policy" ON public.rent_payments FOR SELECT TO authenticated
    USING (landlord_id = auth.uid() OR tenant_id = auth.uid() OR public.is_agency_of_property(property_id, auth.uid()));
...
CREATE POLICY "rent_payments_update_policy" ON public.rent_payments FOR UPDATE TO authenticated
    USING (landlord_id = auth.uid() OR tenant_id = auth.uid() OR public.is_agency_of_property(property_id, auth.uid()));
...
CREATE POLICY "rent_payments_insert_policy" ON public.rent_payments FOR INSERT TO authenticated
    WITH CHECK (landlord_id = auth.uid() OR public.is_agency_of_property(property_id, auth.uid()));
...
-- 4. REALTIME PUBLICATION & REPLICA IDENTITY SETUP
...
-- 6b. STORAGE RLS POLICIES (idempotent — safe to run on production)
...
-- 6.5 LANDLORD OWNERSHIP CLAIM RPC
...
-- 7.1 Add city column to properties
ALTER TABLE public.properties ADD COLUMN IF NOT EXISTS city TEXT;
...
-- 7.3 GENERATE MISSING RENT PAYMENTS (STRICT DEDUPLICATION)
...
-- 8. EMAIL_UNSUBSCRIBES TABLE
-- Abonelikten çıkan e-posta adresleri (2026-08-13)
...
-- 9. AGENCY_REFERRAL_PARTNERS TABLE (2026-08-17)
...
-- Ensure phone column is nullable if table already existed
DO $$
BEGIN
    IF EXISTS (
...
-- 9.4 agency_otp_codes (2026-08-17)
...
-- 10. NOTIFY POSTGREST SCHEMA CACHE
