-- ==============================================================================
-- STANOMER MIGRATION: FIX AGENCY DEMO RENT PAYMENTS STANDARDS
-- Target: DEV ONLY (thvbpifahvasyzmngpzp)
-- Date: 2026-09-12
--
-- Description:
--   1. Normalizes existing rent payments in demo DB: changes receiver_type to 'owner'
--      and title to 'Kira' (matching architectural standard).
--   2. Updates public.generate_agency_demo_data(p_agency_id UUID) to generate
--      complete, rich slice of payments across all 10 properties with 'Kira' / 'owner'.
--   3. Re-runs generate_agency_demo_data for active demo agencies.
-- ==============================================================================

-- 1. Fix any existing rent payments created with legacy demo title/receiver
UPDATE public.rent_payments
SET title = 'Kira',
    receiver_type = 'owner'
WHERE receiver_type = 'agency'
   OR title ILIKE 'zakupnina%'
   OR title ILIKE 'kirija%';

-- 2. Update generate_agency_demo_data Function
...
-- 3. Automatically regenerate payments for any active demo agencies
DO $$
DECLARE
