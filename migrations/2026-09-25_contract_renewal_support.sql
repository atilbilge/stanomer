-- Migration: Contract Renewal Support & Date-Aware Rent Payment Generation
-- Date: 2026-09-25
-- Description:
--   Ensures generate_missing_rent_payments selects the currently effective contract
--   (start_date <= CURRENT_DATE), preventing future renewed contracts from prematurely generating rent rows.
...
BEGIN
  -- Get the ACTIVE contract for the property that applies to CURRENT_DATE (or starts in past/today)
  ...
  WHILE v_current_date <= v_end_date LOOP
    ...
    IF jsonb_array_length(v_expenses_cfg) > 0 THEN
      ...
      LOOP
        ...
        -- B. receiver = 'owner': Kiracı → ev sahibine öder
