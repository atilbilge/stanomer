...
content = re.sub(banner_regex, banner_replacement, content, 1)

# 2. Kontrat Bilgileri Kartı
contract_regex = r'// ── Kontrat Bilgileri Kartı ─────────────────────────────\n\s*if \(activeContract != null\) \.\.\.\[[\s\S]*?\]\s*,\s*\],\s*\),\s*\],'
contract_replacement = """// ── Kontrat Bilgileri Kartı ─────────────────────────────
                if (activeContract != null) ...[
                  const SizedBox(height: 16),
                  Container(
                    decoration: BoxDecoration(
...
content = re.sub(contract_regex, contract_replacement, content, 1)

# 3. Bekleyen davetler
pending_regex = r'// ── Bekleyen Davetler \(Landlord\) ───────────────────────\n\s*if \(effectiveIsLandlord\) \.\.\.\[[\s\S]*?\]\s*,\s*\],\s*\),'
pending_replacement = """// ── Bekleyen Davetler (Landlord) ───────────────────────
                if (effectiveIsLandlord) ...[
                  const SizedBox(height: 16),
                  contractsAsync.when(
                    data: (contracts) {
...
content = re.sub(pending_regex, pending_replacement, content, 1)                

# 4. Navigasyon
nav_regex = r'// ── Navigasyon Linkleri ────────────────────────────────\n\s*const SizedBox\(height: 16\),\n\s*Card\([\s\S]*?\]\s*,\s*\)\s*,\s*\),'
nav_replacement = """// ── Navigasyon Linkleri ────────────────────────────────
                const SizedBox(height: 16),
                if (effectiveIsLandlord)
                  contractsAsync.when(
                    data: (contracts) {
...
content = re.sub(nav_regex, nav_replacement, content, 1)
